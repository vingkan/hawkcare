function renderTable(rows){
	var html = ''
	//Headers
	var headers = ['Image', 'Name'];
	for(var i = 0; i < rows.length; i++){
		if(rows[i]){
			for(var j in rows[i]){
				if(headers.indexOf(j) < 0){
					headers.push(j);
				}
			}
		}
	}
	html += '<tr>'
	for(var h = 0; h < headers.length; h++){
		if(headers[h] === 'Image'){
			html += '<th><div class="dashboard-cell"></div></th>'
		}
		else{
			html += '<th><div class="dashboard-cell">' + headers[h] + '</div></th>'
		}
	}
	html += '</tr>'
	//Rows
	for(var r = 0; r < rows.length; r++){
		var entry = rows[r];
		html += '<tr>'
		for(var v = 0; v < headers.length; v++){
			if(headers[v] === 'Image'){
				html += '<td><div class="dashboard-image" style="background-image: url(&#39;' + (entry[headers[v]] || 'style/img/default-user-image.jpg') + '&#39;);"></div></td>'
			}
			else{
				html += '<td><div class="dashboard-cell">' + (entry[headers[v]] || '---') + '</div></td>'
			}
		}
		html += '</tr>'
	}
	var output = document.getElementById('dashboard');
	output.innerHTML = html;
}

var promises = [];
for(var p = 0; p < 20; p++){
	promises.push(Generator.generateUserPromise());
}
Promise.all(promises).then(function(data){
	var users = data.map(function(obj){
		return obj.results[0];
	});
	var toRender = users.map(function(user){
		return {
			'Image': user.picture.large,
			'Name': String.toTitleCase(user.name.first) + ' ' + String.toTitleCase(user.name.last),
			'Phone': user.phone,
			'Gender': String.toTitleCase(user.gender),
			'City': String.toTitleCase(user.location.city)

		}
	});
	renderTable(toRender);
});

//renderTable([{zik: 2, tik: 3},{zik: 2, tik: 5}]);